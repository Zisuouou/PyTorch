...
def main():
    ...
    model.eval()
    try:
        # 2. Faster R-CNN 전용 더미 데이터 생성
        # 실제 공정 이미지 해상도가 고정되어 있다면 그 크기(예: 800x800)로 지정하는 것이 최적화에 유리합니다.
        # Faster R-CNN의 입력 형태인 List[Tensor] 구조를 맞추어 줍니다.
        dummy_height, dummy_width = 2048, 4096  
        dummy_input = [torch.rand(3, dummy_height, dummy_width, device=device)]
        print(f"[INFO] Tracing 진행 중... (입력 크기: {dummy_height}x{dummy_width})")
        # 3. script 대신 trace를 사용하여 복잡한 모델 내부 레이어를 C++ 바이너리 그래프로 기록
        with torch.no_grad():
