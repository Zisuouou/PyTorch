...
# 예측 돌릴 이미지 폴더
TEST_IMG_DIR = r"C:\Users\SVT\Desktop\PyTorch_PJS\annotations\annos\image"

# 결과 저장 폴더 (하위에 DETECTED / MISSED 자동 생성)
OUT_DIR = r"C:\Users\SVT\Desktop\PyTorch_PJS\runs\results"
...
def run_inference_on_image(model, device, img_path: str, detected_dir: str, missed_dir: str):
    ...
    # 3) 모델 추론 (새로운 FasterRCNNWrapper 규격에 맞게 전면 수정)
    model.eval()
    with torch.no_grad():
        # 대괄호([]) 없이 단일 텐서로 주입합니다.
