...
# 03.19 : PyTorch Profiler 스케줄링 설정 
def trace_handler(prof):
    ...
    # Chrome / Perfetto 용 trace 저장
    trace_path = os.path.join("profiler_logs", f"trace_step_{prof.step_num}.json")
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, writer, print_freq=1):        # 1epoch 학습
    ...
    with profile(
        ...
        schedule=schedule(
            wait=1,  # 1 step 대기
            warmup=1, # 1 step 워밍업
            active=3, # 3 step 프로파일링
            repeat=1  # 이 사이클을 1번 반복 (총 6 step)
        ),
    ...
    # 모든 배치에 대해 순전파 → 손실 계산 → 역전파 → 가중치 업데이트
        ...
        # targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
            ...
            with record_function("backward"):
                optimizer.zero_grad(set_to_none=True)  # 26.03.19_추가 : optimizer.zero_grad(set_to_none=True)로 변경
                losses.backward()
